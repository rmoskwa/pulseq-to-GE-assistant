...
%  PNSwt           [3]             PNS x/y/z/ channel weights. Default: [1 1 1].
%
% Output:
%  W               struct containing the plotted waveforms

arg.timeRange = [0 psq.duration];
arg.blockRange = [1 psq.nMax];
arg.showBlocks = true;
arg.rotate = false;
...
n = 1;    % row counter in psq.loop
nFirst = [];
tic = 0;  % running timer marking start of segment instance (sec)

while n < arg.blockRange(2) & tic - eps < min(psq.duration, arg.timeRange(2))
    ...
    L = psq.loop(n:(n-1+psq.segments(i).nBlocksInSegment), :);

    % get segment instance duration
    try
        S = getsegmentinstance(psq, i, sys, L, 'durationOnly', true);
    catch ME
    ...
    try
        S = getsegmentinstance(psq, i, sys, L, 'rotate', arg.rotate, ...
            'interpolate', arg.interpolate, 'PNSwt', arg.PNSwt);
    catch ME
        error(sprintf('(n = %d, i = %d): %s\n', n, i, ME.message));
    end
