...
%
% Input options
%   PNSwt  [3]     x/y/z/ channel weights. Default: [1 1 1].
%                  From IEC 60601-2-33:2022 section (12): PNSwt = [0.8 1.0 0.7]
%   plt    true/false   plot
%
% Output
...
p = zeros(3, n);
for ch = 1:3
    tmp = arg.PNSwt(ch) * 100 * conv(s(ch,:), f);  % percent of stimulation threshold
    p(ch,:) = tmp(1:n);
end
...
function p_this = sub_test
    ...
    % ground truth
    % to compare, set PNSwt = [1 1 1] above
    %figure;
